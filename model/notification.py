        ...
        try:
            ...
            for rental in rentals:
                ...
                if isinstance(rental_end_str, str):
                    rental_end_date = datetime.strptime(
                        rental_end_str, "%Y-%m-%d"
                    ).date()
                else:
                ...
                if today > rental_end_date:
                    notifications.append(
                        f"OVERDUE: Book '{book_title}' was due on {rental_end_date}. Account Suspended."
                    )
                    should_suspend = True
                ...
                # Check for warning (3 days before)
                    ...
                    notifications.append(
                        f"WARNING: Book '{book_title}' is due in {days_left} days ({rental_end_date})."
                    )

            if should_suspend:
                cursor.execute(
                    "UPDATE student SET isSuspended = 1 WHERE id = ?", (student_id,)
                )
                connection.commit()
