...
def seed_database():
    ...
    with db.connect() as connection:
        try:
            ...
            current.execute("SELECT id FROM student")
            student_ids = [row[0] for row in current.fetchall()]

            current.execute("SELECT id FROM book")
            book_ids = [row[0] for row in current.fetchall()]
            ...
            notifications = [
                (
                    "Your rental for 'The Great Gatsby' is due in 2 days.",
                    student_ids[0],
                ),
                ("Welcome to the library system!", student_ids[1]),
                (
                    "Your account has been suspended due to overdue books.",
                    student_ids[2],
                ),
                (
                    "Reminder: Return 'To Kill a Mockingbird' by tomorrow.",
                    student_ids[3],
                ),
            ]
            ...
            # Print summary
            current.execute("SELECT COUNT(*) FROM student")
            student_count = current.fetchone()[0]

            current.execute("SELECT COUNT(*) FROM book")
            book_count = current.fetchone()[0]

            current.execute("SELECT COUNT(*) FROM rental")
            rental_count = current.fetchone()[0]

            current.execute("SELECT COUNT(*) FROM notification")
            notification_count = current.fetchone()[0]
